...
tau=.99
rtoh_lut<-function(tau)
{
  #ah = rq(log(Hmax) ~ log(radius),  data = b[b$Hmax>20,], tau = tau )
  ah = rq(log(Hmax) ~ log(radius),  data = b, tau = tau )
  aa <- coef(ah)[1]
  bb <- coef(ah)[2]
...
lut<-lapply((1:100)/100, rtoh_lut)
lut<-do.call(rbind, lut)

col_ramp<-colorRampPalette(c("blue", "red"))( 100 )
par(mfrow=c(1,1), mar=c(4,4,2,2))
plot(log(b$radius), log(b$Hmax),pch=19, xlab = "log(Tree height)", ylab = "log(Crown radius)",cex.axis=1.5,cex.lab=1.5)
for(i in 1:100)
  abline(lut$aa[i], lut$bb[i], col=col_ramp[i])


write.csv(lut, 'rtoh_lut_gta.csv')
